...
test('playable smoke path reaches the restoration atlas systems', async ({ page }) => {
  ...
  await expect(page.locator('.greenhouse-visual')).toBeVisible()
  ...
  await expect(page.getByText(/Current save: 0 of \d+ contracts restored; 0 of \d+ Ark systems online/)).toBeVisible()
  await page.getByRole('button', { name: 'Restoration atlas' }).click()
  await expect(page.getByRole('heading', { name: 'Opening Atlas Freedom' })).toBeVisible()
  await expect(page.getByText(/Opening atlas freedom: guided-freedom\. Start with one ready required work order/)).toBeVisible()
  await expect(page.getByText(/Ready required: Training Contract: First Breath/)).toBeVisible()
  await page.getByRole('button', { name: 'Enter active chamber' }).click()
  await page.keyboard.press('Escape')
  await expect(page.getByRole('heading', { name: 'Pause Functions' })).toBeVisible()
  ...
  await expect(page.getByText(/Audio mix\/accessibility pass ready: 7 independent audio mix control\(s\) normalized; caption log, reduced motion, minimal visual mode, high contrast, scan verbosity, text-only hints, remappable keyboard, gamepad support remain surfaced with captioned feedback/)).toBeVisible()
  await expect(page.getByText(/Visual presentation: optional-rich-greenhouse\. Build a richer abstract greenhouse/)).toBeVisible()
  ...
  await expect(page.getByText(/Gamepad input supports left stick or D-pad movement, south button plant\/interact, east button primary scan, west button seed cycling, north button interact\/confirm, shoulder tuning, either trigger plus D-pad/)).toBeVisible()
  ...
  await expect(page.locator('.greenhouse-visual')).toBeHidden()
  ...
  await expect(page.getByRole('heading', { name: 'Packaging and Deployment' })).toBeVisible()
  await expect(page.getByText(/Packaging and deployment ready: Electron is a first-class packaged release target while browser\/static remains the canonical runtime/)).toBeVisible()
  await expect(page.getByText(/electron: first-class packaged release target that loads the canonical dist build/)).toBeVisible()
  ...
  await expect(eventLog.getByText(/Planted voices: .*Persistent planted seed voices active: 1 SeedVoice loop\(s\) for tutorial/)).toBeVisible()
  await page.keyboard.press('z')
  await page.keyboard.press('z')
  await page.keyboard.press('z')
  await page.keyboard.press('b')
  ...
  await expect(page.getByText(/Materials and crafting ready: 7 recipe path\(s\) cover spores tune seed DNA when available; resin locks seed traits/)).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Restored System Rewards' })).toBeVisible()
  await expect(page.getByText(/Restored system rewards ready: 1 online system\(s\) have earned 1 biomass, 1 spores/)).toBeVisible()
  await expect(page.locator('li').filter({ hasText: /Intake: online; 1 of .* contract\(s\) restored; materials .*biomass.*spores/ })).toBeVisible()
  await expect(page.locator('li').filter({ hasText: /Navigation: locked; 0 of .* contract\(s\) restored; materials .*crystal/ })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'Persistent Chamber Changes' })).toBeVisible()
  await expect(page.getByText(/Persistent chamber changes ready: 1 restored chamber change\(s\) preserve ratings, environmental changes, planted seeds, material spend, and seed-move history/)).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'Working Restoration Campaign' })).toBeVisible()
  await expect(page.getByText(/Working restoration campaign: 1 restored, .* available, ratings Resonant 1\. Planning queue .* minute\(s\); 0 rating return option\(s\)\. Next action: restore Contract 1: Intake Lung/)).toBeVisible()
  await expect(page.locator('li').filter({ hasText: /Contract 1: Intake Lung: ready; required; Unrated rating; 5 to 7 minute solve/ })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'Optional Content Pass' })).toBeVisible()
  await expect(page.getByText(/Optional content pass ready: 17 optional contract\(s\) target 15 to 25; 17 advanced, 17 rewarded, 17 progression-gated across 5 season\(s\)/)).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'Opening Atlas Freedom' })).toBeVisible()
  await expect(page.getByText(/Opening atlas freedom: linear-recovery\. 1 ready required work order\(s\), 0 ready optional branch\(es\), 3 queued dependency step\(s\)/)).toBeVisible()
  await expect(page.getByText(/Queued dependency preview: Contract 2: Navigation Grove/)).toBeVisible()
  await expect(page.getByText(/Optional branch preview: Optional Contract: Splice Nursery/)).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Season 1 Contract Structure' })).toBeVisible()
  await expect(page.getByText(/Season 1 contract structure ready: eight required contracts replace the four-chamber prototype, chain Intake Lung through Verdancy Heart, and keep 2 optional branch contract\(s\) available/)).toBeVisible()
  await expect(page.getByText(/Contract 8: Verdancy Heart: finale combination; 9 to 10 minute solve/)).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'End-to-End Progression' })).toBeVisible()
  await expect(page.getByText(/End-to-end progression ready: \d+ main-path contract\(s\) connect Training Contract: First Breath through Contract 24: Heart Atria; finale Contract 8: Verdancy Heart and postgame Optional Contract: Living Conservatory are reachable/)).toBeVisible()
  await expect(page.getByText(/Current live next contract: Contract 1: Intake Lung/)).toBeVisible()
  ...
  await expect(page.getByText(/Visual presentation: minimal mode active; hide the abstract greenhouse layer/)).toBeVisible()
  ...
  await expect(page.getByText(/Intake, Navigation, Water, and Canopy systems ready: 4 authored contracts with unlocks; 1 online in the current save/)).toBeVisible()
  ...
  await expect(page.getByText(/First codex records ready: Training Contract: First Breath rewards 6 readable record\(s\)/)).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'First Endings' })).toBeVisible()
  await expect(page.getByText(/Ending choice model: both\. Explicit resolution choices select the final scene/)).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'Seed Library Grafting' })).toBeVisible()
  ...
  await expect(page.locator('p').filter({ hasText: /Selected seed DNA: .*Synth type: (pure|fm|am|noise-kissed); routes to .* seed voice/ })).toBeVisible()
  ...
  await expect(page.locator('p').filter({ hasText: /Selected seed DNA: .*Noise profile: .*breath, compost, and masking texture.*routes through .* procedural/ })).toBeVisible()
  ...
  await expect(page.getByText(/Codex complete ready: 96 authored record\(s\) target 80 to 120; \d+ contract reward record id\(s\) resolve; 9 record tree\(s\) cover 96 record\(s\)/)).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: 'Story Payoff', exact: true })).toBeVisible()
  await expect(page.getByText(/Story payoff: 1 of 8 arc record\(s\) recovered; next protected silence when you restore Quiet Mirror to learn why absence matters/)).toBeVisible()
  await expect(page.getByText(/Caretaker identity: The first restored breath identifies the player as the Ark caretaker/)).toBeVisible()
...
test('uses Enter to plant interact and confirm after solving', async ({ page }) => {
  ...
  await expect(eventLog.getByText(/Persistent planted seed voices active: 1 SeedVoice loop\(s\) for tutorial/)).toBeVisible()
  await expect(eventLog.getByText(/Training Contract: First Breath solved with Resonant rating/)).toBeVisible()
  await page.keyboard.press('Enter')
  await expect(page.getByRole('heading', { name: 'Restoration Atlas' })).toBeVisible()
})
...
test('opens the postgame conservatory when unlocked', async ({ page }) => {
  ...
  await expect(page.getByRole('navigation', { name: 'Ending resolution choices' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Preservation recommended' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Conservatory selected' })).toBeVisible()
  await expect(page.getByText(/First endings ready: Conservatory is playable now; 4 authored resolution scene\(s\) are present/)).toBeVisible()
