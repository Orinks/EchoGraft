...
test('playable smoke path reaches the restoration atlas systems', async ({ page }) => {
  ...
  await expect(page.getByText(/Syngen gamepad input supports left stick or D-pad movement, south button plant\/interact, east button primary scan, west button seed cycling, north button interact\/confirm, shoulder tuning/)).toBeVisible()
  ...
  await expect(eventLog.getByText(/Gamepad: left stick or D-pad moves, south button plants\/interacts, east button scans, west button cycles seeds, north button interacts\/confirms, shoulder buttons tune/)).toBeVisible()
