...
function scan() {
  ...
  if (scanMode === 'materials') feedback = materialScanState(chamber, save).text
  if (scanMode === 'chamber') feedback = chamberChangeScanState(chamber, save, plantedSeeds).text
...
function boundaryObjectiveScanV1Html() {
  const state = boundaryObjectiveScanV1State(player, chamber, save)
  return `
    <section aria-labelledby="scan-v1-title">
...
function cycleScanMode() {
  scanMode = scanLogModes[(scanLogModes.indexOf(scanMode) + 1) % scanLogModes.length]
...
function atlas() {
  ...
  const workingCampaign = workingRestorationCampaignState(chambers, save, save.arkClock)
  ...
  const firstFourSystems = firstFourArkSystemsState(chambers, save)
  ...
  shell(`
    <main class="screen atlas" aria-labelledby="atlas-title">
      ...
      <section aria-labelledby="working-campaign-title">
        <h2 id="working-campaign-title">Working Restoration Campaign</h2>
        <p>${workingCampaign.text}</p>
        <ol>${workingCampaign.entries.map((entry) => `<li>${entry.text}</li>`).join('')}</ol>
      </section>
      ...
      <section aria-labelledby="main-chamber-catalog-title">
        <h2 id="main-chamber-catalog-title">40+ Main Chambers</h2>
        <p>${mainChamberCatalog.text}</p>
        <ol>${mainChamberCatalog.seasons.map((season) => `<li>${season.text}</li>`).join('')}</ol>
      </section>
      <section aria-labelledby="optional-content-title">
        <h2 id="optional-content-title">Optional Content Pass</h2>
        <p>${optionalContent.text}</p>
        <ol>${optionalContent.seasons.map((season) => `<li>${season.text}</li>`).join('')}</ol>
      </section>
      ...
      <section aria-labelledby="progression-title">
        <h2 id="progression-title">End-to-End Progression</h2>
        <p>${endToEnd.text}</p>
        <p>Current live next contract: ${endToEnd.nextLive?.title ?? 'all authored contracts complete'}.</p>
        <ol>${endToEnd.mainPath.map((item) => `<li>${item.title}: Season ${item.season}, ${item.contractType}, ${item.system}.</li>`).join('')}</ol>
      </section>
      ...
      <section aria-labelledby="first-four-systems-title">
        <h2 id="first-four-systems-title">Intake Navigation Water Canopy Systems</h2>
        <p>${firstFourSystems.text}</p>
        <ol>
          ${firstFourSystems.systems.map((system) => `<li>${system.text}</li>`).join('')}
        </ol>
      </section>
      ...
      <ol class="contract-list">
        ${chambers.map((item) => {
          ...
          return `<li>
            ...
            <p>${teachingAxisSummary(item).text}</p>
            <p>${axisCombinationMasterySummary(item, save).text}</p>
            <p>${optionalComplexitySummary(item).text}</p>
...
function library() {
  ...
  shell(`
    <main class="screen" aria-labelledby="library-title">
      ...
      <section aria-labelledby="seed-library-menu-title">
        <h2 id="seed-library-menu-title">Seed Library Menu</h2>
        <p>${menuState.text}</p>
        <ol>${menuState.sections.map((section) => `<li>${section.text}</li>`).join('')}</ol>
      </section>
...
function graftingScreen() {
  audio.setMusicScene('menu')
  const fullCatalog = fullSeedGraftCatalogState()
  const graftComplexity = graftGeneticsApproachabilityState()
  const menuState = seedLibraryMenuState(inventory, save, selectedSeedIndex)
  shell(`
    <main class="screen" aria-labelledby="grafting-screen-title">
      ...
      <section aria-labelledby="grafting-catalog-title">
        ...
        <p>${graftComplexity.text}</p>
        <ol>${graftComplexity.visibleRuleGroups.map((rule) => `<li>${rule}.</li>`).join('')}</ol>
        <p>${fullCatalog.text}</p>
...
function codex() {
  ...
  const arkOrigin = arkOriginMysteryState(save)
  const storyPayoff = codexStoryPayoffState(save, availableCodexRecords())
  shell(`
    <main class="screen" aria-labelledby="codex-title">
      ...
      <section aria-labelledby="story-payoff-title">
        <h2 id="story-payoff-title">Story Payoff</h2>
        <p>${storyPayoff.text}</p>
        <ol>${storyPayoff.entries.map((entry) => `<li>${entry.text}</li>`).join('')}</ol>
      </section>
...
function help() {
  ...
  shell(`
    <main class="screen" aria-labelledby="help-title">
      ...
      <p>${controlsText()}</p>
      <section aria-labelledby="manual-complete-title">
        <h2 id="manual-complete-title">Manual Complete</h2>
        <p>${manual.text}</p>
      </section>
      <section aria-labelledby="screen-reader-testing-title">
      ...
      ${manual.sections.map((section) => `<section aria-labelledby="manual-${section.id}"><h2 id="manual-${section.id}">${section.title}</h2><p>${section.text}</p></section>`).join('')}
      <button data-action="game">Back to game</button>
      ${captionLogHtml()}
    </main>
  `)
