...
function seedDnaText(seed) {
  return `${seedNameState(seed).text} ${seedFamilyState(seed).text} ${seedPitchRatioState(seed, chamber.target.pitchRatio).text} ${seedPulseRateState(seed, chamber.target.pulseRate).text} ${seedBrightnessState(seed, chamber.target.brightness).text} Phase ${seed.phase}, envelope attack ${seed.envelope?.attack}, release ${seed.envelope?.release}, FM ${seed.fmAmount}, AM ${seed.amAmount}, noise ${seed.noiseAmount}, growth ${seed.growthBehavior}. ${seedLineageText(seed)} ${historicalSeedTraitState(seed, save).text}`
}
